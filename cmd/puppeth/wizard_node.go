...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.Genesis.Config.Wabash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			fmt.Printf("Where should the wabash mining DAGs be stored on the remote machine?\n")
			infos.ethashdir = w.readString()
		} else {
			fmt.Printf("Where should the wabash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ethashdir)
	...
	if !boot {
		if w.conf.Genesis.Config.Wabash != nil {
			// Wabash based miners only need an etherbase to mine against
			fmt.Println()
			if infos.etherbase == "" {
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Hap node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
