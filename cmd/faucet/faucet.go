...
// faucet is a Hap faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Hap protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Hap and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Hap address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Hap light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Hap protocol stack
	client *ethclient.Client   // Client connection to the Hap chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Hap light client protocol
...
// close terminates the Hap connection and tears down the faucet.
...
// apiHandler handles requests for Hap grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Hap address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/wabei/go-hap/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Hap address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Hap addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Hap address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Hap address and profile URL.
...
// returning the username, avatar URL and Hap address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Hap address,
