...
type wallet struct {
	...
	deriveChain    wabei.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain wabei.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
